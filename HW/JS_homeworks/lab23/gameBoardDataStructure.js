...
function setX(row,column) {
	if(row < board2d.length && column < board2d[row].length){
		board2d[row][column] = `x`
	}	
}
...
// 	if(row < board2d.length && column < board2d[row].length	){
// 		//current 'x' where????
// 		const columnOfOldX = board2d[row].indexOf('x')
// 		board2d[row][columnOfOldX] = 0
// //------------------------------------------------------
// 		board2d[row][column] = `x`//future
